...
require APPPATH . 'vendor/autoload.php'; # composer class loader
...
require 'Adobe/Config/AppConfig.php';
...
class Adobe 
{
    public static function search($query) 
    {
        ...
        $search_params->setWords($query['keyword']);
        $search_params->setLimit($query['limit']);
        $search_params->setOffset($query['offset']);
        ...
        echo "Displaying first results\n";
        # get files array from search and iterate over results
        Adobe::displayResults($response->getFiles());
    ...
    static function displayResults($response_files)
    {
        for ($idx = 0; $idx < count($response_files)-1; $idx++) {

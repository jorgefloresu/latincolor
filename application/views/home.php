...
<nav class="top-content white-text z-depth-0" role="navigation">
  <div class="nav-wrapper">
      ...
      <a href="https://api.whatsapp.com/send?l=es&phone=573142958463&text=Buen%20dia,%20tengo%20una%20consulta" target="_blank" class="phone" style="padding-left: 30px;">
        <img src="<?=base_url('img/whatsapp-30.png')?>" style="margin: 10px -35px;position: absolute;"> + (57) 314 295 8463</a>
      <div id="signuser" class="sesion-menu">
        <?php if ($logged): ?>
          <a href="<?=site_url('main/user')?>" class="sign-in modal-trigger">Hola, <?=$user_data->first_name?></a>
          <span><i class="fas fa-user-circle fa-lg"></i></span>
          <a href="<?=site_url('login/logout/noindex')?>" id="sign-out">Cerrar sesión</a>
          <a href="#signup-page" id="signup-user" class="modal-trigger" style="display:none"><?=_("Sign up")?></a>
        <?php else: ?>
          <a href="#sign-in" class="sign-in modal-trigger"><?=_("Login")?></a>
          <span><i class="fas fa-user-circle fa-lg"></i></span>
          <a href="<?=site_url('login/logout/noindex')?>" id="sign-out" style="display:none">Cerrar sesión</a>
          <a href="#signup-page" id="signup-user" class="modal-trigger"><?=_("Sign up")?></a>
        <?php endif; ?>
      </div>
  ...
  <div class="nav-wrapper" style="top:5px">
    <div class="row">
      ...
      <div class="col s2">
        <ul class="right hide-on-med-and-down">
          <li>
            <a id="menuCart" href="#" class="menu-cart white-text tooltipped" data-position="bottom" data-delay="50" data-tooltip="Lista vacía">
        ...
        <ul id="slide-out" class="left-nav side-nav">
          <li><div class="user-view">
                ...
                <a href="#!name"><span class="white-text name"><?=$user_data->fname?></span></a>
                <a href="#!email"><span class="white-text email"><?=$user_data->email_address?></span></a>
              </div>
          </li>
          <li><a href="<?=site_url('main/planes')?>">Planes & Suscripciones</a></li>
        ...
        <?=form_open('main/search/1', 'id="home-search" method="get"')?>
          <div class="search-wrapper card">
              <div class="input-field">
                <input id="search" type="search" name="keyword" placeholder="Buscar imágenes, vectores y videos" required>

var Api = ( function() {
  ...
  var showResult = function(data) {
            ...
            searchFields(data);
              ...
              // prepare(data, function(arr){
              //   var grid = setup.config.resultSearch.isotope({
              ...
              //       // }
              //     });
              //   let items = '<div class="grid-sizer"></div>';
 ...
 $.prepare = function(data) {
   return $.Deferred(function(defer){
     ...
     $.each(data.result, function(index, item) {
       $.addImageCards(item).done(function(res){
         //console.log(res);
         items.push(res);
       })
     });
...
var prepare = function(data, callback) {
    let count = 0;
    let arr = [];
     ...
     for(let item of data.result) {
       ...
       img.onload = function () {
         count++;
         let div = (img.height > img.width) ? "<div class='grid-item grid-item--height2'>" : "<div class='grid-item'>";
         arr[count-1] = div +
                        data.result[count-1].html.replace("_w_",img.width).replace("_h_",img.height)+
                        "</div>";
         if (count==data.result.length) callback(arr);
       }
       img.src = item.thumb;
     };
 }

 var prepare2 = function(data, callback) {
     let count = 0;
     let arr = [];
      ...
      for(let item of data.result) {
        ...
        img.onload = img.onerror = function () {
          ...
          arr[count-1] = "<div>" +
                         data.result[count-1].html.replace("_w_",img.width).replace("_h_",img.height)+
                         "<div class='caption'>"+item.id+Templates.cardAction(data.preview+item.id,item.provider)+"</div>"+
                         "</div>";
          if (count==data.result.length) callback(arr);
        }
  ...
  $.addImageCards = function(item){
            var result = function(res) {
              ...
              img.onload = function () {
                //items.h = img.height;
                //items.w = img.width;
                var hdom = "<div class='grid-item'><img src='"+item.thumb+"' height='"+img.height+"' width='"+img.width+"' /></div>";
                res.resolve(hdom);
              }
              //console.log(img.height);
          }
            //setup.config.resultSearch.append($(items));
            return $.Deferred(result).promise();
          };
